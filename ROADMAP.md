...
### 1. Хвіст ARCH — 3 інлайн-запити в `telegram.ts` (лишився ОДИН файл)

- **Ціль:** `check-repo-layer.mjs` каже «no inline SQL in worker/routes» — тоді C1 стає повною
  забороною, а не храповиком.
- **Стан (2026-08-07):** було 10 → **3**. `import.ts` (3) і `setup.ts` (2) поїхали після
  `worker/test/integrations.test.ts` (11 сценаріїв, non-vacuous: підміна валюти рахунку валить 2,
  `INSERT OR REPLACE` замість `IGNORE` валить 2). `webhook.ts` (2) — після того, як
  `ingest.test.ts` доріс до 11 сценаріїв.
- **Що лишилось:** три `SELECT id, name FROM categories …` у `telegram.ts` — будівники inline-
  клавіатур. Читання, без записів, тож ризик не той, що на інжесті; але точка входу не покрита
  нічим.
- **Чому незручно:** хендлери керуються update-пейлоадом, а не HTTP-роутом, і відповідь бот шле
  через `fetch` у Telegram API. Тобто харнесу потрібен мок `fetch` + знімок ВИХІДНИХ викликів
  (`sendMessage`/`editMessageText` із клавіатурою) — саме вони і є спостережувана поведінка.
- **Кроки:** 1) мок `fetch` + сценарій на `POST /tg/<secret>` з `callback_query` (`tgcat`) і
  з `/balance`; 2) перенести три запити в `repo/categories.ts`; 3) видалити рядок із BUDGET.
- **Готово-коли:** 0 `.prepare()` поза `repo/`, усі знімки зелені.

### 2. `src/pages/Stats.tsx` — 1 379 рядків

Найбільший файл проєкту; клієнт свідомо лишався поза межами ARCH. Розрізати за вкладками.
⚠️ Спершу подивитись, чи не потягне це за собою `Stats`-специфічні типи — контракт уже в `shared/api/`.

### 3. Тех-борг — два писарі таблиці `facts`
...
- **[нове] Web-Push (PWA) (M).** Свідомо відкладено: VAPID-ключі, перехід vite-plugin-pwa на
  ...
  Web Crypto у Worker. На iOS працює лише для PWA на «Додому». Зараз ту саму роль грає TG-пуш.
...
- **[нове] Шейр-таргет для ФОТО чека (M).** Файловий share-target МУСИТЬ бути `method: "POST"`, а
  POST доставляється у fetch-хендлер service worker'а — тобто перехід на `injectManifest` і власний
  SW. Той самий розмін, що вже відкладено для Web-Push.
...
- **[нове] Бекап/відновлення (M).** Експорт-імпорт усього стану у файл; ручний бекап у R2.
  ⚠️ Зараз бекапів НЕМА взагалі — це усвідомлена межа, записана в `SECURITY.md`.
