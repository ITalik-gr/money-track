...
### Разовий прохід §ENRICH-GATE по вже наявних списаннях

**Ціль:** підписка, що списувалась ДО 2026-09-10, теж має іконку — без того, щоб власник тиснув
«Розпізнати» на кожній.
**Корінь:** `enrich-gate.ts` виправив ворота ВЕБХУКА, тобто все нове. Історія лишилась як була:
`enrichPending` (кнопка «Розпізнати все») досі вибирає лише `category_id IS NULL`, тож списання,
яке MCC-правило колись розклало, у той батч не потрапляє ніколи. Самолікується за один біллінговий
цикл (наступне списання того ж мерчанта піде через `ask`), тож це не терміново — але поки цикл не
минув, §SUB-DETECT бачить менше, ніж міг би.
**Файли:** `worker/lib/ai/enrich.ts` (`enrichPending`), `worker/lib/ai/enrich-gate.ts`,
`worker/routes/api/jobs.ts`.
**Готово-коли:** батч бере й рядки з категорією, пропускаючи їх через `enrichVerdict`, і на реєстрі
власника не робить жодного зайвого виклику на продуктах і переказах.
