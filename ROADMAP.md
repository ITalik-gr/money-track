...
### 3. Безпека — два хвости

- ⬜ **`/security-review` по НОВІЙ структурі.** Периметр переїхав разом із кодом, а аудит
  2026-07-26 закривав діри саме в тих місцях, які тепер перенесені.
- ⬜ **Повний скан історії git секрет-сканером** (`gitleaks`/`trufflehog`, усі гілки).
  `git log -S` — не доказ, він матчить один патерн із багатьох.
- ⬜ `.dev.vars.example` — звірити, що повний і без реальних значень.
- *(Рішення власника закриті 2026-08-03: історію git не переписуємо; `database_id` лишається
  в `wrangler.jsonc`. Деталі — `ARCHITECTURE.md §6`.)*
