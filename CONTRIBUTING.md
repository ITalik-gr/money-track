...
```bash
npm run check    # tsc (app + worker) + SQL lint + i18n lint + migration embed check + tests
npm run build
```
...
3. **Layering:** `routes/*` is transport and validation; logic lives in `lib/*`; no SQL in
   components. (A refactor is currently pulling raw SQL out of the route layer — see
   `ARCHITECTURE.md`. Don't add new SQL to `routes/`.)
