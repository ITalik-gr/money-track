...
- **§ENRICH-GATE (2026-09-10): вебхук питає модель про те, що НЕВІДОМО, а не про те, що без
  категорії — `lib/ai/enrich-gate.ts`.**
  Випадок власника: списання підписки Apple на 44,35 ₴; MCC-правило поклало його в «Сервіси, SaaS
  продукти» — правильно; і воно так і лишилось звичайною покупкою, поки він РУКАМИ не натиснув
  «Розпізнати», після чого застосунок зрозумів, що це підписка, і намалював іконку. Ворота вебхука
  були `category_id IS NULL`, тобто питали «чи розклали правила», а `ai_recurring` (єдине джерело
  тієї іконки й вхід для §SUB-DETECT) пише ЛИШЕ enrich. Дві правильні поведінки, і між ними діра.
  Питання, на яке ворота мусять відповідати, — «чи лишилось що дізнатись»: правила вміють сказати,
  ЩО ЦЕ ЗА КРАМНИЦЯ, і не вміють сказати, чи воно повторюється, чи назва — бренд чи банківський шум,
  і до якого з оголошених планів воно належить.
  ⚠️ **Відповідь — не «збагачувати все».** Власник назвав форму точно: «не всі, а оце часті по типу
  продуктів, між картками, округлення балансу не розпізнавало через аі, бо там і так все зрозуміло».
  Тож `enrichVerdict` має ТРИ відповіді, і коштує лише одна:
  **`skip`** — клас, де детермінований висновок повний: рух власних коштів (пара, `is_transfer`,
  `descriptionIsTransfer`, бакет 13), дохід, і `OBVIOUS_MCC` — продукти, кафе, пальне, транспорт,
  аптека, банкомат. Критерій потрапляння в той список потрійний: категорія однозначна з самого коду,
  у назві немає бренду, який варто чистити, **і під цим MCC не буває підписки** — саме останнє
  робить пропуск безпечним. Стрімінг, софт, хмара, телеком, зал і страхування у списку відсутні
  СВІДОМО.
  **`carry`** — цього мерчанта вже збагачували, тож вердикт КОПІЮЄТЬСЯ (безкоштовно, і це типовий
  випадок підписки: друге списання Apple питати не треба). Матч — точна назва мерчанта або точний
  сирий опис банку, навмисно ВУЖЧЕ за нечіткий корінь `consensusCategory`: той відповідає «яку
  категорію дають схожі списання» (де приблизно правильно — корисно), а цей — «чи це підписка», де
  приблизно правильно про ІНШОГО мерчанта поставило б фальшиву іконку, яку ніхто не подумає
  перевіряти. Історію питають ПЕРЕД `OBVIOUS_MCC`: кафе, що продає місячний абонемент, стоїть під
  5812, і список інакше змахнув би єдине списання, яке справді повторюється.
  **`ask`** — справді нове, або категорії немає взагалі (старі ворота, не ослаблені).
  ⚠️ **Тут не вирішується КАТЕГОРІЯ.** `skip` лишає `ai_recurring` НЕЗАЙМАНИМ, а не пише 0: колонка
  навмисно розрізняє «спитали, і ні» від «ніколи не питали» (міграція 0046), і ворота, які
  відповідають замість моделі, зробили б ці два стани однаковими — після чого повернутись і глянути
...
- **§ADVICE-LOOP (2026-09-04): порадник перестав бути монологом.**
  Було: `Advice.suggestions` — 3-5 кроків, і `AdviceAction` мав рівно ОДИН варіант
  (`create_budget`), тож чотири кроки з пʼяти були прозою, що нікуди не веде. А
  `pushAdviceHistory` зберігав `summary` й числа й **викидав самі поради**, тому питання «що
  порадник радив місяць тому і чи це спрацювало» не мало відповіді в даних — не важкої, жодної.
  §NOVELTY тримав від повторів СТРІЧКУ; порада такої охорони не мала й могла прийти втретє,
  не знаючи, що вона третя.
  - **Ідентичність поради — нормалізований ЗАГОЛОВОК, ніколи не індекс у масиві.**
    Порада перегенеровується цілком, тож індекс означав би, що стан, поставлений на другу
    пораду, наступного разу ляже на те, що модель поставить другим — мовчки й на вигляд
    правильно. `advice-history.ts` уже вивчив це для знімків («індекс зсунеться при видаленні з
    іншого пристрою»), тут наслідки гірші.
    ⚠️ **Перефразована порада — це НОВА порада**, і це чесно, а не недогляд: нечіткий ключ, що
    злив би дві справді різні поради, сховав би другу назавжди. Від перефразування захищає інше —
    модель БАЧИТЬ, що вже казала.
  - **`rememberSuggestions` вставляє, але не перезаписує.** Інакше кожна генерація скидала б усі
    «не треба» в `open`, і застосунок тихо скасовував би відмови користувача, а тоді знову
    пропонував відхилене — рівно та поведінка, заради якої сховище й заведено.
  - **§NOVELTY для порад:** у payload їде `already_suggested` — заголовки, стани й, де виміряно,
    `outcome_delta_pct`. Промт забороняє подавати `dismissed` удруге. Правило те саме, що в
    стрічці: **повторюваність лікується ЯВНИМ списком уже сказаного.**
  - **Результат ВИМІРЮЄТЬСЯ, а не стверджується.** `scoreTakenSuggestions` бере канонічний рівень
    категорії (`categoryMonthlyLevels`, переданий зі знімка — не порахований удруге, бо число під
    порадою мусить стояти на тому самому каноні, що й порада над ним) і рахує дельту від
    `baseline`, знятого в день поради. Модель числа не дає: «доставка впала на 15%» — рівно та
    підбадьорлива фраза, яку модель напише незалежно від того, чи це правда (правило
    `numbersAreGrounded`).
    ⚠️ Базова величина знімається ОДИН раз. Перезняття на кожній зміні стану дало б базу «сьогодні»,
    проти якої покращення нульове за побудовою — вимір, що завжди згоден сам із собою.
    ⚠️ Не раніше 25 днів; `dismissed` не оцінюється НІКОЛИ (оцінювати людину за планом, від якого
    вона відмовилась, — не вимір, а докір), `open` — теж (там нема чого міряти).
  - **`AdviceAction` розширено до пʼяти типів,** і кожен має ВИКОНАВЦЯ, що вже існував:
    `create_budget`/`set_budget` → `setBudget`, `create_goal` → `createGoal`,
    `cancel_subscription` → `deletePlanned`, `create_rule` → `createRule`. Закритий union, а не
    вільний рядок: **дія без виконавця — це кнопка, яка бреше**, і це гірше за абзац, який чесно
    є абзацом.
    ⚠️ Позначка `taken` ставиться, навіть якщо мутація впала: людина ВИРІШИЛА, і памʼятаємо саме
    рішення. Інакше той, хто спіймав тимчасову помилку, отримав би ту саму пораду наступного
    місяця так, ніби нічого не було.
  - **Стани мержаться на ВИДАЧІ** (`normaliseSuggestions` у роуті), а не пишуться в збережену
    пораду: позначка, поставлена секунду тому, видно на пораді, згенерованій годину тому, і
    позначання не переписує блоб — інакше два пристрої, що позначають одночасно, губили б одне з
    двох рішень.
  - Порада, згенерована ДО 2026-09-04, не має `key`/`state`; читачі це терплять, бо
    `normaliseSuggestions` дає дефолти на виході.
