...
- **Вагомість (§6):** `EFF_IMPORTANCE` = `COALESCE(t.importance, рол-ап importance ефективної категорії, 'discretionary')`. Рівні `essential|discretionary|optional`. Задається на категорії (дефолт) + override на транзакції. Міграція 0016.
  ⚠️ `t.ai_importance` (migration 0054, docs/JEV.md phases 3–4) is NOT in this COALESCE and must
  not be added to it: it is a model's PROPOSAL. Measured at 97% since it is asked with the root
  known (§7.4), it is OFFERED beside the importance picker where it differs from the category —
  and becomes canon only through `t.importance`, i.e. a person's click and save.
...
- **Бюджети-конверти (ліміт ↔ витрачено ↔ прогноз) — `budgetStatus(env, mult, now)`**
  ...
  - **§BUDGET-PACE (2026-09-10): «за поточним темпом» мусить бути про поточний темп.**
    `projected` — це БЛЕНД поточного темпу з канонічним рівнем категорії, зважений на початку
    місяця майже цілком у бік історії. Тому конверт, у якому цього місяця **не витрачено нічого**,
    10-го числа все одно проєктується приблизно на 48% рівня — і власник отримав за один ранок дві
    події: «„Здоровʼя“ іде на 227% бюджету — За поточним темпом місяць закриється на 682 ₴ при
    ліміті 300 ₴. Витрачено поки що 0 ₴». Кожне число в них пораховано правильно, місяць —
    календарний (spend-вікно завжди було `localMonthStart`), а речення все одно неправдиве: те, що
    воно називає, дорівнювало нулю.
    Тому `BudgetStatusRow` віддає **`pace_ratio`** — де місяць закриється на витратах САМОГО цього
    місяця (`spent / elapsedFrac`), без жодної історії, — і `draftBudgetForecast` вимагає
    `pace_ratio ≥ 1` на додачу до решти воріт.
    ⚠️ **Конверт, чий РІВЕНЬ вищий за ліміт, — це інша знахідка, і застосунок уже робить її двічі**
    (§BUDGET-REACH `unreachable`, §ENV-PARTS `floor_over_limit`). Переказувати її щодня голосом
    прогнозу темпу означало б утретє сперечатись про ті самі гроші — і саме тим тоном, який не
    підходить. Тримається `budget-pace.test.ts` (мовчання, голос, і те, що §BUDGET-REACH не зник).
  - **§ENV-PARTS (2026-09-04): конверт каже, З ЧОГО він складається, а не лише скільки зʼїдено.**
    `envelopeParts` (`lib/finance/envelope-parts.ts`) ділить витрату місяця на три частини:
    **`committed`** — списання, привʼязані до оголошеного плану (§PLAN-LINK, `transactions.planned_id`);
    **`rhythmic`** — не привʼязані ні до чого, але мерчант має ВИМІРЯНИЙ ритм (§SUB-DETECT
    `recurringCandidates`), тобто підписка, якої ніхто не оголосив; **`discretionary`** — решта,
    єдина частина, яку рішення ще може змінити.
    ⚠️ **Три частини в СУМІ дають `spent`** — вони його частини, ніколи не доданки. Та сама
    дисципліна, що `recurring + lumpy === monthly_burn` у §BURN-SHAPE, і з тієї ж причини: читач,
    який може скласти їх неправильно, колись складе. Тримається `envelope-parts.test.ts`.
    ⚠️ **Третього визначення «регулярного» тут НЕ зʼявляється.** На це питання в застосунку вже є
    дві відповіді, і обидві навмисні — §PLAN-LINK (що людина ОГОЛОСИЛА) і §SUB-DETECT (що реєстр
    ВИМІРЯВ). Файл лише питає їх і додає. Своє правило дало б два різні числа про одну підписку на
    сторінці Плану й на сторінці Підписок — вада, яку закривали §CUR-PLAN і §SUB-MONTH.
    ⚠️ **Вікно ритму — 400 днів, те саме, що в `/planned/detect`.** Коротше означало б, що План
    називає списання дискреційним, поки Підписки пропонують зробити з його мерчанта підписку.
    ⚠️ Запит звужено до мерчантів, які витрачали САМЕ цього місяця: очевидна форма (вантажити всю
    400-денну історію, як робить `/planned/detect`) коштує 4 000 рядків на шляху, яким ходить і
    добовий `notify.draftBudgets`, а група без списання цього місяця не може змінити тут жодної цифри.
  - **§ENV-PARTS, друга половина: `floor` і §BUDGET-REACH ПОСЕРЕД місяця.**
    `floor = committed + rhythmic` — гроші, які конверт винен ще до будь-якого рішення. Якщо
    `floor > amount` (ефективний ліміт), ставиться `floor_over_limit`: ліміт уже недосяжний, і
    стриманість до кінця місяця його не поверне.
    ⚠️ Це НЕ дублікат `unreachable`. Той порівнює ліміт із **рівнем** категорії, тобто з
    шестимісячним вимірюванням, і каже «ця ціль ніколи не досягалась». Цей — про списання, що вже
    впали ЦЬОГО місяця, тож правдивий уже 3-го числа, тоді як звіт закриття правдивий 1-го
    наступного. Два питання, дві назви.
    ⚠️ Проти ЕФЕКТИВНОГО ліміту, не базового: перенесення — це реальні гроші, які конверт може
    витратити (§BUDGET-MEMORY), і без нього застосунок повідомляв би про пробій, якого немає.
    ⚠️ Нульовий конверт — ніколи (§BUDGET-ZERO), той самий виняток, що й у §BUDGET-REACH.
  - **§ENV-PARTS, третя половина: підлога ЗАСТОСОВУЄТЬСЯ до AI-плану, а не згадується в промті.**
    `proposeBudgets` тепер несе в payload `committed_floor_uah` на категорію — місячний тягар
    ОГОЛОШЕНИХ планів (§SUB-MONTH `monthlyPlannedUAH`, єдине джерело) — **і піднімає пропозицію
    моделі до цієї підлоги в коді** (`Math.max`). Модель не бачить `planned_payments`, а середнє
    ховає зобовʼязання за побудовою — та сама причина, з якої їй передають `lumpy`.
    ⚠️ Підлога береться з РОЗКЛАДУ, а не з живого `floor` конверта: той рахується від початку
    місяця, тож 3-го числа підписка, що списується 20-го, важить у ньому нуль. Ліміт — це
    твердження про цілий місяць, і зіставляти його треба із зобовʼязанням цілого місяця.
    ⚠️ **Лише піднімає, ніколи не опускає.** Пропозиція ВИЩЕ підлоги — судження про дискреційні
    витрати, і воно моделі належить; нижче — арифметична помилка. Без цього автобюджет відтворював
    би саме ту скаргу §BUDGET-REACH, заради якої існує: конверт, що читає «153% перевищено» щомісяця
    за ціль, недосяжну арифметично.
