...
- **§MCP-OAUTH (2026-08-23, directory 0010): цей деплой САМ став authorization server —
  ...
  ⚠️ **Готовність до ДРУГОГО клієнта (2026-09-02).** Досі підключався лише Claude, тож усе, що
  «мало б працювати», ніколи не перевірялось іншим кодом. Чотири краї закрито наперед, і кожен із
  них ламається МОВЧКИ:
  - **`iss` у кожній відповіді авторизації (RFC 9207)** — і в успішній, і в ПОМИЛКОВІЙ. Клієнт, що
    говорить із кількома серверами, інакше не знає, хто відповів. ⚠️ Прапорець
    `authorization_response_iss_parameter_supported` і реальне відправлення — це ПАРА, і жодна
    половина не безпечна окремо: рекламувати без відправлення = строгий клієнт відхиляє КОЖНУ
    відповідь; відправляти без реклами = клієнт, який умів би перевірити, не знає, що можна.
    `iss` додається в `backToClient`, а не на кожному виклику, щоб гілка помилки не могла його
    забути — відхилене як зіпсоване пояснення це пояснення, якого людина не побачить.
  - **`/.well-known/openid-configuration` як АЛІАС** того самого документа RFC 8414. OIDC-discovery
    старша за 8414 на роки, тож клієнт із OIDC-звичками пробує її ПЕРШОЮ і на 404 каже «не можу
    дістатись сервера» — той самий мовчазний симптом, що вже описаний вище. ⚠️ Вигадувати
    OIDC-поля (`userinfo_endpoint`) не можна: це реклама неіснуючих ендпоінтів, і клієнт зламається
    ПІЗНІШЕ й далі від причини.
  - **Преflight (`OPTIONS`) на `/.well-known/*` і на `/mcp`, і `WWW-Authenticate` у
    `access-control-expose-headers`.** Клієнт шле `MCP-Protocol-Version` (так велить специфікація),
    тобто запит стає непростим, браузер питає дозволу — а 404 на цей OPTIONS убиває fetch ще до
    справжнього запиту. На сервер не приходить нічого. Преflight на `/mcp` стоїть ПЕРЕД гардом:
    інакше на нього відповідає 401, який гард і має видавати, і справжній виклик не станеться.
    `WWW-Authenticate` — це весь вказівник, куди йти автентифікуватись, і браузерний клієнт не
    може його ПРОЧИТАТИ, поки він не exposed.
  - **Схеми інструментів — строгі явно** (`strict()` у `chat-tools.ts`, і те саме руками для
    `get_finance_snapshot`, який оголошений в іншому файлі). `required` присутній ЗАВЖДИ, хоч би й
    порожній, плюс `additionalProperties: false`. Схема, яку клієнт відхилив, означає для нього, що
    інструмента НЕ ІСНУЄ: помилки не буде ніде, асистент просто відповідатиме так, ніби не бачить
    книги. ⚠️ Усі поля в `required` НЕ заганяються (цього хоче strict-режим OpenAI): це зробило б
    справді необовʼязкові фільтри обовʼязковими для КОЖНОГО клієнта, включно з внутрішнім чатом.
  Тримається `oauth.test.ts` (форма редіректу другого клієнта — поруч із Claude, разом із CSP
  сторінки згоди для його оріджина) і `mcp.test.ts` (строгість кожної схеми, включно зі снапшотом).
