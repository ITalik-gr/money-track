...
- **Клієнт:** усі UI-рядки через `t()` (`src/i18n`); теги локалі лише в `src/i18n/locale.ts`
  ...
  тримає парність `en`/`uk`, забороняє хардкод-тег, сирий `new Intl.*` та рантайм-ключ із
  НЕоголошеним префіксом — і від 2026-09-18 **ключ, якого ніщо не досягає** (26 таких видалено
  того ж дня; чек додано САМЕ ТОМУ, що список став нулем — лінт із дозволеним списком на 26
  позицій вчить додавати двадцять сьому). Мертвий рядок гірший за зайву вагу: перекладача просять
  тримати в актуальності речення з екрана, який його більше не каже. НЕ перекладати: коментарі, `brands.tsx`,
