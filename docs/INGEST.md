...
- **§CSV-DEBIT (2026-09-02): виписка носить суму У ДВОХ ФОРМАХ, і другу читали НЕПРАВИЛЬНО.**
  ...
  - **ДВІ колонки** («Дебет»/«Кредит», обидві ДОДАТНІ), і знак живе в тому, ЯКА заповнена. Це
    велика родина: українські банківські експорти й практично будь-який бухгалтерський CSV.
  ⚠️ **Друга форма не просто не підтримувалась — вона мовчки читалась НАВИВОРІТ.** У списку підказок
  `amount` лежали `"debit"` і `"credit"`, тож такий файл мапився на ту з них, що трапилась першою,
  і імпортував ПОЛОВИНУ рядків — кожен із перевернутим знаком, бо `250,00` у колонці «Дебет» — це
  гроші, що ПІШЛИ. Нічого не падало. Превʼю, чия єдина робота — спіймати не ту колонку суми до
  того, як місяць хибних чисел потрапить у канон, показувало впевнену й хибну відповідь.
  ⚠️ **Знак береться з КОЛОНКИ, не з клітинки** (`-Math.abs`, ніколи не заперечення): банк, який
  друкує дебет уже відʼємним, каже те саме, і заперечення перетворило б витрату на дохід саме для
  тих банків, що були явними.
  ⚠️ **Порожня клітинка з одного боку — НЕ помилка розбору:** так двоколонковий реєстр каже «цей
  рядок іншого роду». Пропускається лише рядок, порожній з ОБОХ боків.
  ⚠️ **Знакова колонка ПЕРЕМАГАЄ пару:** вона поширеніша й однозначна, тож пара навіть не питається.
  ⚠️ **Самотня колонка КРЕДИТ не вгадується взагалі** (знайдено власним тестом): «credit» частково
  збігається з «Credit card number» і «Credit limit» — обидва реальні, обидва парсяться як число, —
  тож здогад поставив би сумою номер картки й упевнено імпортував нісенітницю. Незамапана `amount` —
  краща відмова: вона ВИДИМА (спрацьовує §CSV-AI, а не спрацює — людині показують вибір колонок).
  Самотній ДЕБЕТ вгадується: «Витрата»/«Списання»/«Debit» ні з чим не конфліктують.
  ⚠️ Колонку «Надходження» видно й у веб-пікері: коли вона заповнена, «Сума» МІНЯЄ значення, і
  людина, чий файл гадалка прочитала не тією формою, інакше не може цього сказати.
  ⚠️ Таблицю підказок заодно розширено під звичайні українські та європейські заголовки
  («Дата проводки», «Призначення платежу», «Booking date», «Narrative», «CCY»…). Нерозпізнаний
  заголовок коштує виклик моделі (§CSV-AI), підказка — нічого й відтворювано. **Нічого настільки
  загального, як `"name"`:** воно частково збігається з «Account name» і показало б опис на колонку
  рахунку в кожному експорті, що її має. Тримається `csv-shapes.test.ts` (10 сценаріїв).
