...
export function formatDate(unix: number): string {
  return new Intl.DateTimeFormat(localeTag(getLocale()), { day: "2-digit", month: "short" }).format(unix * 1000);
}
...
export function monthShort(monthIndex0: number): string {
  return new Intl.DateTimeFormat(localeTag(getLocale()), { month: "short" }).format(new Date(2021, monthIndex0, 1));
}
