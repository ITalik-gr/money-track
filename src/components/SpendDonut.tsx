...
function DonutTip(props: any, sign: string) {
  ...
  return (
    <div className="chart-tip">
      ...
      <div className="r" style={{ color: "rgba(255,255,255,0.6)" }}>{d.payload.pct}% витрат</div>
    </div>
  );
}
...
export function SpendDonut({ rows, sign }: { rows: Overview["byCategory"]; sign: string }) {
  ...
  const top = primary.slice(0, 7).map((r, i) => ({ name: r.category_name ?? "без категорії", value: r.spent, color: r.color ?? FALLBACK[i % FALLBACK.length] }));
