...
import { generateFinancialReport, type FinancialReport } from "./report-prompt.ts";
...
export async function generateAndStoreReport(
  ...
  const { result, usage } = await generateFinancialReport(env, {
    // Capped at 12, not 24 (2026-08-12): three reports' worth of topics covers most of what is true
    // about a stable budget, and the model complied by saying less. `prior_reports` keeps its three.
    ...(context as object), prior_reports: priorSummaries, already_covered: [...covered].slice(0, 12),
  });
  ...
  /**
   * Evidence about the ANSWER, stored beside it (2026-08-12).
   *
   * The owner reported that reports "have gone small", and there was no way to tell whether that
   * was the model being terse, an output limit, or the §NOVELTY suppression list leaving nothing
   * to say. A complaint about size that cannot be checked is an argument; these four numbers turn
   * it into a fact. Costs nothing — they are counted from what we already have.
   *
   * In `data_json` rather than new columns: it is diagnostic metadata, not something any query
   * filters or sorts by, and a migration for a debugging aid is a poor trade.
   */
  const diag = {
    sections: result.sections?.length ?? 0,
    advice: result.advice?.length ?? 0,
    anomalies: result.anomalies?.length ?? 0,
    // What the model was FORBIDDEN from presenting as news. If this is high while `sections` is
    // low, §NOVELTY is the reason the report is short — the exact hypothesis this records.
    suppressed: Math.min(covered.size, 12),
