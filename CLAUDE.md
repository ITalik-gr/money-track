...
**Компенсацію видно в UI + підготовка до публічного репо (2026-07-27, 0 міграцій).**
- **🐞 «Увімкнув компенсацію, а всюди повна ціна».** Канон рахував ПРАВИЛЬНО — прогнав саме ті
  числа (витрата 1375 ₴, компенсовано 1000 ₴): статистика й категорія дають 375 ₴, дохід 0.
  Баг був у видимості: список і шапка деталей показували суму, яку списав БАНК, тож користувач,
  щойно записавши компенсацію, бачив те саме число й робив висновок, що не спрацювало.
  Тепер обидва місця ведуть `amount + reimbursed` (те саме, що `EFF_AMOUNT`), а списану суму
  лишають під нею закресленою — обидва факти правдиві, і потрібні обидва. Кейс додано в тести.
  **Правило: якщо канон коригує число, UI мусить показувати САМЕ скориговане** — інакше вірна
  математика читається як зламана фіча.
- **Публічний репозиторій:** додано `LICENSE` (MIT); пошту власника прибрано з документів
  (лишилась лише у Worker-секреті); `graphify-out/` розтречено й у `.gitignore` — це локальний
  індекс, що перегенеровується на кожну зміну коду, тобто ~12 МБ нового блоба в історію щоразу;
  скріншоти стиснуто 4590px/9 МБ → 1836px/872 КБ у `docs/screenshots/` (оригінали лишились на
  диску поза git). Перевірено й чисто: жодного секрета в трекнутих файлах, `.dev.vars` у ignore,
  збіги на `sk-ant-` — плейсхолдер у підказці UI.
...
## ➡️ Що далі — ПЛАТФОРМА-ФАЗА (✅ ЗАПУЩЕНО, стан → `HISTORY.md`)
**Проєкт став публічним портфоліо** («зроблено за допомоги AI») і **живе в проді з 2026-07-26**.
Стан фази й здобуті заміри — `HISTORY.md`; дизайн/ТЗ — `HISTORY.md`; картки — `ROADMAP.md §Черга`.
