...
> (ARCH, фази 0–5) закритий**, стилі розділені на сімнадцять файлів (лінти C8/C9),
> **реєстрація ВІДКРИТА** і тільки через Google; Telegram — ДРУГИЙ ключ до наявного акаунта
> (§TG-MINIAPP), не спосіб його створити.
> Міграції: `finance` до **0046**, `directory` до **0010**. Тестів — **795**.
...
- `worker/lib/ai/` — усе модельне, ШАРАМИ (2026-08-07, було 1335 рядків в одному `ai.ts`):
  ...
  `chat-tools`↔`advisor` замкнули б цикл імпортів) · **`transfers-ai.ts`** (§F2 крок 2 — що
...
Стан на 2026-08-24: міграції `finance` **до 0046**, `directory` **до 0010** (0036–**0046** і directory
