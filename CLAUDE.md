...
> Міграції: `finance` до **0048**, `directory` до **0010**. Тестів — **884**.
> Оновлено 2026-09-04.
...
- `src/styles/*` — двадцять доменних файлів (`advisor.css` — від 2026-08-27, шов C8:
  `domains-a.css` знову вперлась у стелю, а виняток рости не може); `src/index.css` — ЛИШЕ імпорти (лінт C8), і порядок імпортів У НЬОМУ — це каскад:
