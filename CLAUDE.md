...
> Оновлено 2026-07-31. Стан: **у проді**, платформа-фаза закрита, вхід тільки Google,
> **реєстрація ВІДКРИТА** (§Ops п.4). Міграції: `finance` до **0034**, `directory` до **0004**.
...
Усі попередні блокери зняті: міграції `finance` накочені **до 0034**, `directory` — **до 0004**
(0004 = лічильники активності для адмін-таба, накотити перед деплоєм цієї зміни),
D1 `directory` = `c72e2571-1fbb-44b2-8308-5a961aef9670`, секрети на місці, деплой зроблено.
Історія блокерів більше не потрібна — вона в `HISTORY.md`.
⚠️ **Щоб відкрита реєстрація справді працювала — Google Cloud треба ОПУБЛІКУВАТИ** (кнопка
«Publish app»). Поки застосунок у **Testing**, Google пускає лише пошти зі списку Test users, і
`SIGNUP=open` у коді для сторонніх нічого не змінює. Скоупи `openid`/`email`/`profile`
non-sensitive, тож publish не потребує верифікації.
...
4. **Вхід — тільки Google. Реєстрація ВІДКРИТА (2026-07-31).** Будь-хто з Google-акаунтом
   ...
   ⚠️ **Google Cloud у режимі Testing пускає лише пошти з Test users** — поки застосунок не
   опубліковано, «відкрита реєстрація» в коді нічого не змінює для сторонніх.
