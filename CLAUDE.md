...
> (ARCH, фази 0–5) закритий**, стилі розділені на чотирнадцять файлів (лінти C8/C9),
> **реєстрація ВІДКРИТА** і тільки через Google; Telegram — ДРУГИЙ ключ до наявного акаунта
> (§TG-MINIAPP), не спосіб його створити.
> Міграції: `finance` до **0045**, `directory` до **0008**. Тестів — **652**.
...
- `src/styles/*` — чотирнадцять доменних файлів; `src/index.css` — ЛИШЕ імпорти (лінт C8). Токени —
  `styles/tokens.css`. `analytics.css` — шов від 2026-08-21: `domains-a.css` уперлась у стелю, а
  виняток рости не може, тож нові блоки Статистики їдуть туди.
...
Стан на 2026-08-21: міграції `finance` **до 0045**, `directory` **до 0008** (0036–0045 і directory
0006–**0008** — накотити перед деплоєм; **0008 `tg_links` обовʼязкова, інакше бот не маршрутизує
нікого, крім власника** — див. HISTORY 2026-08-21),
